...
function getSessionId(callback) {
    const self = this;
    const { sessionId } = this.globalData;
    if (sessionId) {
...
export function getUserInfo(callback) {
    ...
    const { userInfo } = self.globalData;
    if (userInfo) {
...
export function regist() {
    const self = this;
    const { iv, encryptedData, sessionId } = self.globalData;
    post('/wechat/decodeUserInfo/', { iv, sessionId, encryptedData }, (e, res) => {
        if (e) {
